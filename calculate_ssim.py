...
def get_ssim(level, task, strategy):
...
def get_runs(level, task, strategy):
    runs = api.runs("colin-cooke/ctc", {
        "$and": [{"state": "finished"}, {"config.task": task},
                 {"config.level": level}, {"config.init_strategy": strategy}, {"config.noise": None}]})
    ids = []
...
def predict(run_id, level, task):
    ...
    avg_psnr = np.mean([measure.compare_ssim(im_true, im_test) for im_true, im_test in zip(y_true, preds)])
    return avg_psnr
...
if __name__ == "__main__":
    ...
    out_dir = os.path.join('/hddraid5/data/colin/ctc/ssims')
    os.makedirs(out_dir, exist_ok=True)
    for task in tasks:
        for strategy in strategies:
            psnrs = np.array([get_ssim(level, task, strategy) for level in levels])
